...
def test_msgpack_env_strict_after_msgpack_import(
    caplog: pytest.LogCaptureFixture, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Strict msgpack env is evaluated when the serializer is constructed."""
    monkeypatch.setattr(_lg_msgpack, "STRICT_MSGPACK_ENABLED", False)
    monkeypatch.setenv("LANGGRAPH_STRICT_MSGPACK", "true")
    serde = JsonPlusSerializer()
